...
.stApp {
    background:
        ...
        rgba(20, 220, 140, 0.10),
        transparent 25%),
        linear-gradient(135deg, #070b10 0%, #0b1118 50%, #080d12 100%);
    color: #F5F7FA;
}
...
.winner {
    color: #28e39f !important;
    font-weight: 900 !important;

    background-color:
        rgba(40, 227, 159, 0.08);
}
...
# If a player has a different name between the
# old and new file, add him here.
#
# Example:
#
# "new name": "old name"
...
def find_old_player(new_row):
    ...
    name_key = new_row["Name_Key"]

    matches = old_df[
        old_df["Name_Key"] == name_key
    ]
    ...
    if name_key in normalized_aliases:
        ...
        old_key = (
            normalized_aliases[name_key]
        )

        matches = old_df[
            old_df["Name_Key"] == old_key
        ]
...
st.markdown(
    ...
    <div class="hero">
        ...
        <div class="hero-text">
            ...
            Yaya's Rating is a fantasy-focused player
            rating built to identify the most valuable
            EuroLeague players.

            <br>

            The score combines production, efficiency,
            consistency, playing time, price value and
            the player's role within his team.

            <br>

            Go beyond the basic Fantasy average and
            find the players who can give you the edge.
...
with h2h_tab:
    ...
    def comparison_row(
        ...
        left_class = winner_class(
            value1,
            value2,
            1
        )

        right_class = winner_class(
            value1,
            value2,
            2
        )
        ...
        left_value = format_number(
            value1,
            decimals
        )

        right_value = format_number(
            value2,
            decimals
        )
    ...
    player1_display = display_name(
        player1
    )

    player2_display = display_name(
        player2
    )
